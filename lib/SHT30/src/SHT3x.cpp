...
// bool SHT3x::SHT30_SD(String currentDate, String currentTime){
// 	UpdateData();
// 	uint8_t tem = GetTemperature();
//   	uint8_t hum = GetRelHumidity();
  
//   	String data = "Temperature: " + String(tem) + ", Humidity: " + String(hum);

//  	if (sdHelper.writeToFile("/SHT30Data.txt", data)) {
//     Serial.println("Dữ liệu SHT30 đã được ghi vào thẻ SD !!!  \n");
//     return true;
//   	} else {
...
bool SHT3x::SHT3x_SD(String currentDate, String currentTime) {
  UpdateData();
  uint8_t tem = GetTemperature();
  uint8_t hum = GetRelHumidity();
  if (currentDate != lastDate3) {
    lastDate3 = currentDate;
    String dateData = "\nDate: " + currentDate;
    if (sdHelper.writeToFile("/SHT3xData.txt", dateData)) {
      Serial.println("Ngày mới đã được ghi vào thẻ SD !!!");
    } else {
      Serial.println("[ERROR] Không thể ghi ngày vào thẻ SD.");
    	}
  	}
  	String data = currentTime + ":   Temperature: " + String(tem) + ", Humidity: " + String(hum);
  	if (sdHelper.writeToFile("/SHT3xData.txt", data)) {
    Serial.println("Dữ liệu SHT3x đã được ghi vào thẻ SD !!!");
    return true;
  	} else {
    Serial.println("[ERROR] Không thể ghi dữ liệu SHT3x vào thẻ SD.");
    return false;
  	}
}
